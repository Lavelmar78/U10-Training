// ─────────────────────────────────────────────────────────────────
// DEBUG BUILD — shows any loading error directly on the page.
// Once things are working, switch back to the normal
// firebase-client.js (remove this wrapper).
// ─────────────────────────────────────────────────────────────────
try {
...
window.DrillAppCloud = {
  ...
  banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:#c0392b;color:white;padding:14px;font-family:monospace;font-size:13px;white-space:pre-wrap;word-break:break-word;';
  banner.textContent = 'FIREBASE-CLIENT.JS ERROR:\n' + (err && err.message ? err.message : err) + '\n\nStack:\n' + (err && err.stack ? err.stack : 'n/a');
  document.addEventListener('DOMContentLoaded', () => document.body.prepend(banner));
  if (document.body) document.body.prepend(banner);
  console.error('firebase-client.js failed to load:', err);
}
